 ...
 * Adapted from Stefan Gustavson's SimplexNoise.
 */
...
export class SimplexNoise {
  constructor(seed = Math.random()) {
    ...
    const s = this._xorshift(seed);
    for (let i = 0; i < 256; i++) this.p[i] = i;
    for (let i = 255; i > 0; i--) {
      const n = (this._xorshift(s + i) & 0x7fffffff) % (i + 1);
      const tmp = this.p[i];
      this.p[i] = this.p[n];
      this.p[n] = tmp;
    }
  ...
  _xorshift(val) {
    val = Math.imul(val ^ (val >>> 16), 0x45d9f3b);
    val = Math.imul(val ^ (val >>> 13), 0x45d9f3b);
    return (val ^ (val >>> 16)) >>> 0;
  }
